    ...
    try:
        ...
        with open(audio_file_path, "rb") as audio_file:
            ...
            transcription = client.audio.transcriptions.create(
                file=audio_file,
                model=model,
                language="en",  # Specify English for better accuracy for English learning
                temperature=0.0,  # Use default temperature for best accuracy
                response_format="json"  # Use json for simple text response
            )
...
async def convert_audio_to_text_with_metadata(
    ...
    try:
        ...
        with open(audio_file_path, "rb") as audio_file:
            # Use Groq Whisper for transcription with verbose output
            transcription = client.audio.transcriptions.create(
                file=audio_file,
                model=model,
                language="en",
...
async def convert_audio_to_text_optimized(audio_file_path: str, context: str = "") -> Optional[str]:
    ...
    try:
        ...
        # Use whisper-large-v3 for better accuracy when context is important
